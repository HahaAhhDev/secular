...
import { buildIndex, identify } from "./detect.js";
import { loadCatalog } from "./spdx.js";
import { getMeta } from "./meta.js";
import { evaluate, complianceScore } from "./rules.js";
import { parsePackageJson, parseCargoToml, parseGoMod, parsePyproject, parseGemfile, parseComposerJson, } from "./manifests.js";
export async function scan(opts) {
    ...
    const projectLicenses = new Map();
    const thirdParty = new Map();
    const deps = [];
    const isRootLicense = (f) => {
        const base = path.basename(f.rel);
        if (path.dirname(f.rel) !== ".")
            return false;
    ...
    for (const f of files) {
        ...
        if (hit.id) {
            if (isRootLicense(f)) {
                ...
                if (existing)
                    existing.files.push(f.rel);
                else
                    thirdParty.set(hit.id, { category: getMeta(hit.id).category, files: [f.rel] });
            }
        }
    }
    ...
    for (const f of files) {
        if (f.kind !== "source" || headersChecked > 3000)
            continue;
        let head;
        try {
        ...
        if (!/(SPDX-License-Identifier|Copyright|Licensed under|Licensed under the)/i.test(head))
            continue;
        headersChecked++;
    ...
    ctx.hasNoticeFile = hasNoticeFile;
...
function parseManifest(f) {
    const base = path.basename(f.abs);
    try {
        if (base === "package.json")
            return parsePackageJson(f.abs, f.rel);
        if (base === "Cargo.toml")
            return parseCargoToml(f.abs, f.rel);
        if (base === "go.mod")
            return parseGoMod(f.abs, f.rel);
        if (base === "pyproject.toml")
            return parsePyproject(f.abs, f.rel);
        if (base === "Gemfile")
            return parseGemfile(f.abs, f.rel);
        if (base === "composer.json")
            return parseComposerJson(f.abs, f.rel);
    }
    catch {
        /* ignore */
    }
...
export function heuristicNetworkService(root) {
    try {
        const pkg = path.join(root, "package.json");
        if (fs.existsSync(pkg)) {
            const p = JSON.parse(fs.readFileSync(pkg, "utf8"));
            const all = { ...(p.dependencies ?? {}), ...(p.devDependencies ?? {}) };
            if (all.express || all.fastify || all.koa || all["@hono/node-server"] || all.next || all.nuxt)
                return true;
        }
        const req = path.join(root, "requirements.txt");
        if (fs.existsSync(req)) {
            const t = fs.readFileSync(req, "utf8");
            if (/flask|django|fastapi|uvicorn|starlette/i.test(t))
                return true;
        }
        const go = path.join(root, "go.mod");
        if (fs.existsSync(go)) {
            const t = fs.readFileSync(go, "utf8");
            if (/gin-gonic|echo|fiber|chi|gorilla\/mux/i.test(t))
                return true;
        }
    }
    catch {
