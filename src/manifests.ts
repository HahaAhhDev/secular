...
export function parsePyproject(abs: string, source: string): Dep[] {
  try {
    ...
    let inDeps = false;
    for (const line of text.split(/\r?\n/)) {
      if (/^\s*\[/.test(line)) { inDeps = /^\s*\[(?:project\.)?dependencies\]/.test(line); continue; }
      if (!inDeps) continue;
      const m = line.match(/^\s*"?\)?([A-Za-z0-9_.\[\]-]+)\s*(?:[=<>!~])/);
      if (m) deps.push({ name: m[1]!.split("[")[0]!.trim(), scope: "dependency", ecosystem: "pypi", source });
      else {
        const name = line.match(/^\s*([A-Za-z0-9_.-]+)\s*$/);
        if (name) deps.push({ name: name[1]!, scope: "dependency", ecosystem: "pypi", source });
      }
    }
    return deps;
  } catch {
...
export function parseGemfile(abs: string, source: string): Dep[] {
  try {
    ...
    for (const line of text.split(/\r?\n/)) {
      ...
      if (m) deps.push({ name: m[1]!, scope: "dependency", ecosystem: "rubygems", source });
    }
    return deps;
  } catch {
    return [];
